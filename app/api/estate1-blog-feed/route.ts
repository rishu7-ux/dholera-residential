...
function toRichContentHtml(value: unknown): string | null {
  ...
  try {
    ...
    const hasVisibleContent = html.replace(/<[^>]*>/g, "").replace(/&nbsp;/g, " ").trim().length > 0 || /<(img|hr|iframe|video)\b/i.test(html);
    return hasVisibleContent ? html : null;
  } catch (error) {
    console.error("Estate 1 rich content could not be serialised", error);
    return null;
  }
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await payload.find({
      ...
      collection: "estate1-blogs" as never,
      where: slug ? { and: [{ slug: { equals: slug } }, { status: { equals: "published" } }] } : { status: { equals: "published" } },
      sort: "-publishedAt",
      limit: slug ? 1 : limit,
      depth: 1,
    });
    const blogs = (result.docs as unknown as Estate1BlogRecord[]).map((blog) => {
    ...
    console.error("Estate 1 blogs could not be loaded", error);
