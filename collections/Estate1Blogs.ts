...
export const Estate1Blogs: CollectionConfig = {
  slug: "estate1-blogs",
  labels: { singular: "Estate 1 Blog", plural: "Estate 1 Blogs" },
  admin: {
    useAsTitle: "title",
    group: "Dholera Estate 1 Content",
  ...
  fields: [
    { name: "title", label: "Blog Title", type: "text", required: true },
    {
      ...
      hooks: {
        beforeValidate: [({ value, siblingData }) => {
          ...
          return String(source).toLowerCase().trim().replace(/\//g, "-").replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "").replace(/-+/g, "-").replace(/^-|-$/g, "");
        }],
      },
    },
    ...
    { name: "content", label: "Blog Content", type: "textarea", required: true },
    {
      ...
      type: "richText",
      admin: { description: "Optional structured article content. Use H2 for major sections and H3 for subsections; the Blog Title is the page H1." },
      editor: lexicalEditor({
        features: () => [
          ParagraphFeature(), HeadingFeature({ enabledHeadingSizes: ["h2", "h3"] }), BoldFeature(), ItalicFeature(), OrderedListFeature(), UnorderedListFeature(), LinkFeature({ enabledCollections: [] }), FixedToolbarFeature(), InlineToolbarFeature(),
        ],
      }),
    },
